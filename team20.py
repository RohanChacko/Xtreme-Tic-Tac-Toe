...
class Team20(object):
    ...
    def update(self, move, player, tog_update, normal_board):
        ...
        if tog_update:
            normal_board.big_boards_status[move[0]][move[1]][move[2]] = player
        ...
        bs = normal_board.big_boards_status[k]
        for i in range(3):
            #checking for horizontal pattern(i'th row)
            if (bs[3*x+i][3*y] == bs[3*x+i][3*y+1] == bs[3*x+i][3*y+2]) and (bs[3*x+i][3*y] == player):
                normal_board.small_boards_status[k][x][y] = player

                if not tog_update:
                    normal_board.small_boards_status[k][x][y] = '-'
                    normal_board.big_boards_status[move[0]][move[1]][move[2]] = '-'
                return
        ...
        if not tog_update:
            normal_board.big_boards_status[move[0]][move[1]][move[2]] = '-'
            if normal_board.small_boards_status[k][x][y] == 'd':
                normal_board.small_boards_status[k][x][y] = '-'
        ...
        if tog_update:
            normal_board.small_boards_status[k][x][y] = 'd'

        return

    def minimax(self, depth, old_move, is_maximizing_player, alpha, beta, move_history):
        ...
        if depth == 0 or self.board.find_terminal_state() != ('CONTINUE', '-'):
            ...
            if zobhash in self.transposition_table:
                ...
                state_score = self.evaluate_heuristic(self.board, old_move, depth, is_maximizing_player, move_history)
        ...
        if is_maximizing_player:
            ...
            for current_move in available_moves:
                ...
                self.update(current_move, self.player, 1, self.board)

                move_history.append([current_move, self.player])
                ...
                if self.board.small_boards_status[0][allowed_small_board[0]][allowed_small_board[1]] != '-' and self.board.small_boards_status[1][allowed_small_board[0]][allowed_small_board[1]] != '-':
                    value = self.minimax(depth-1, current_move, True, alpha, beta, move_history)
                else:
                    value = self.minimax(depth-1, current_move, False, alpha, beta, move_history)

                best_heuristic_val = max(best_heuristic_val, value)
                alpha = max(alpha, best_heuristic_val)
                self.update(current_move, self.player, 0, self.board)
            ...
            for current_move in available_moves:
                ...
                self.update(current_move, self.opponent, 1, self.board)
                move_history.append([current_move, self.opponent])
                ...
                self.update(current_move, self.opponent, 0, self.board)
    ...
    def evaluate_heuristic(self, board, old_move, depth, is_maximizing_player, move_history):
        ...
        state_score += self.heur_b(self.board)
        #################################### Heuristic C ####################################
        ...
        #################################### Heuristic D Move History ####################################

        move_history.reverse()
        board_copy = deepcopy(self.board)
        for move in move_history:

            if board_copy.find_terminal_state() == (self.player,'WON'):
                state_score += (sys.maxint + depth)/100
            elif  board_copy.find_terminal_state() == (self.opponent,'WON'):
                state_score += -1*sys.maxint - depth
            
            
            state_score += self.heur_b(board_copy)
            self.update(move[0], move[1], 0, board_copy)
    ...
    def heur_b(self, bs):
        ...
        for k in range(2):
            for i in range(3):
                for j in range(3):
                    if bs.small_boards_status[k][i][j] == self.player:
                        count += 150
                    if bs.small_boards_status[k][i][j] == self.opponent:
                        count -=200

        for k in range(0,2):

            count += 200 * self.eval_smallboard(k, self.player, bs)
            count -= 250 * self.eval_smallboard(k, self.opponent, bs)

        return count

    def eval_smallboard(self, board_num, player, small_board_1):
        ...
        bs = small_board_1.small_boards_status[board_num]
        for i in range(0,3):
            for j in range(0,3):
    ...
    def evaluate_big_board(self):
        ...
        player_adv_pos_count = 10*self.get_big_board_advantage_positions(self.player)
        opponent_adv_pos_count = 15*self.get_big_board_advantage_positions(self.opponent)
                ...
                for j in range(0,9,3):
                    for l in range(3):
                        if big_board_weight[k][i+l][j] +  big_board_weight[k][i+l][j+1] + big_board_weight[k][i+l][j+2] == 2:
                            advantange_pos_count += 3
                        if big_board_weight[k][i+l][j] +  big_board_weight[k][i+l][j+1] + big_board_weight[k][i+l][j+2] == 3:
                            advantange_pos_count += 4
                        if big_board_weight[k][i+l][j] +  big_board_weight[k][i+l][j+1] + big_board_weight[k][i+l][j+2] == -1:
                            advantange_pos_count += 2
                        if big_board_weight[k][i][j+l] +  big_board_weight[k][i+1][j+l] + big_board_weight[k][i+2][j+l] == 2:
                            advantange_pos_count += 3
                        if big_board_weight[k][i][j+l] +  big_board_weight[k][i+1][j+l] + big_board_weight[k][i+2][j+l] == 3:
                            advantange_pos_count += 4
                    ...
                    if big_board_weight[k][i][j] + big_board_weight[k][i+1][j+1] + big_board_weight[k][i+2][j+2] == 2:
                        advantange_pos_count += 3
                    if big_board_weight[k][i][j] + big_board_weight[k][i+1][j+1] + big_board_weight[k][i+2][j+2] == 3:
                        advantange_pos_count += 4
                    if big_board_weight[k][i][j] + big_board_weight[k][i+1][j+1] + big_board_weight[k][i+2][j+2] == -1:
                        advantange_pos_count += 2
                    if big_board_weight[k][i][j+2] + big_board_weight[k][i+1][j+1] + big_board_weight[k][i+2][j] == 2:
                        advantange_pos_count += 3
                    if big_board_weight[k][i][j+2] + big_board_weight[k][i+1][j+1] + big_board_weight[k][i+2][j] == 3:
                        advantange_pos_count += 4
                    if big_board_weight[k][i][j+2] + big_board_weight[k][i+1][j+1] + big_board_weight[k][i+2][j] == -1:
                        advantange_pos_count += 2
