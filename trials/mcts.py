...
class MCTS:
    ...
    def __init__(self, n_iterations=1500, depth=162, exploration_constant=5.0, tree=None, win_mark=3, game_board=None, player=None):
        ...
        while not leaf_node_found:
            ...
            if n_child == 0:
                ...
                for i in range(n_child):
                    action = self.tree[node_id]['child'][i][0]
                    ...
                    exploration_value = math.sqrt(math.log(total_n) / n)
    ...
    def expansion(self, leaf_node_id, old_move):
        '''
        ...
        winner = leaf_state.find_terminal_state()
        possible_actions = leaf_state.find_valid_move_cells(old_move)
        ...
        if winner[0] == 'CONTINUE':
            ...
            for action_set in possible_actions:
                ...
                if current_player == 'o':
                    next_turn = 'x'
                    state.big_boards_status[action[0]
                                            ][action[1]][action[2]] = 1
                else:
                    next_turn = 'o'
                    state.big_boards_status[action[0]
                ...
                self.tree[child_id] = {'state': state,
                                       ...
                                       'parent': leaf_node_id,
                                       'n': 0, 'w': 0, 'q': 0}
                self.tree[leaf_node_id]['child'].append([action_idx, action])
                action_idx += 1
            # print "length error: " + str(len(childs))
            if len(childs) == 1:
                rand_idx = 0
            else:
                rand_idx = random.randint(0, len(childs) - 1)
            # print len(childs)
            # print rand_idx
    ...
    def simulation(self, child_node_id, old_move):
        '''
        ...
        while not anybody_win:
            winner = state.find_terminal_state()
            if winner is not None:
                anybody_win = True
            else:
                possible_actions = state.find_valid_move_cells(old_move)
    ...
    def backprop(self, child_node_id, winner):
        ...
        player = copy.deepcopy(self.tree[(0,)]['player'])
    ...
    def move(self, game_board, old_move, flg):
        for i in range(self.n_iterations):
            leaf_node_id, depth_searched = self.selection()
            child_node_id = self.expansion(leaf_node_id, old_move)
            winner = self.simulation(child_node_id, old_move)
...
def gameplay(obj2):  # game simulator
...
if __name__ == '__main__':
    ...
    if option == '1':
        ...
        obj2 = Random_Player()
        # obj2 = Manual_Player()
        # obj1 = MCTS(n_iterations=1500, depth=15, exploration_constant=100, game_board=None, player='x')
    elif option == '3':
        obj1 = Manual_Player()
        obj2 = Manual_Player()
    else:
